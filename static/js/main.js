...
function renderArticleCard(article) {
  ...
  const articleUrl = `${baseUrl}/${article.url}`.replace(/\/\/+/g, '/'); // Combine and normalize slashes
  const blogUrl = `${baseUrl}/blog`.replace(/\/\/+/g, '/');
          ...
          setTimeout(() => {
            lazyImages.forEach(img => {
              if ((img.getBoundingClientRect().top <= window.innerHeight && img.getBoundingClientRect().bottom >= 0) && 
                ...
                lazyImages = Array.prototype.filter.call(lazyImages, 
                  function(image) { return image !== img; }
                );
...
function initInfiniteScroll() {
  ...
  async function init() {
    try {
      ...
      const indexUrl = `${baseUrl}/content/index.json`.replace(/\/\/+/g, '/');
    ...
    try {
      ...
      articlesToLoad.forEach(article => {
        if (typeof window.renderArticleCard === 'function') {
            ...
            const fallbackUrl = `${baseUrl}/${article.url}`.replace(/\/\/+/g, '/');
...
function initShareMenuToggle() {
  var toggleBtns = document.querySelectorAll('.share-toggle-btn');
  toggleBtns.forEach(function(toggleBtn) {
    ...
    toggleBtn.addEventListener('click', function(e) {
      e.stopPropagation();
      var expanded = menu.classList.toggle('share-menu-open');
